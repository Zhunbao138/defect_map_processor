...
document.getElementById('upload-form').addEventListener('submit', async (e) => {
    ...
    const recognition = document.querySelector('input[name="recognition"]:checked')?.value || 'ocr';
    ...
    try {
        ...
        // cscan 可能返回多个 task_ids (每 sheet 一个)
        ...
        if (taskIds.length > 1) {
            alert(`已为 ${taskIds.length} 个日期 sheet 各创建任务，开始处理...`);
        }
        showProgressSection();
        startPolling();
        refreshTaskList();
...
function buildRecordCardHTML(rec) {
    ...
    const imgUrl1 = rec['图-1'] ? `/api/image/${(rec && rec._task_id) || currentTaskId}/${relPath(rec['图-1'])}` : null;
    const imgUrl2 = rec['图-2'] ? `/api/image/${(rec && rec._task_id) || currentTaskId}/${relPath(rec['图-2'])}` : null;
...
function renderImageItem(path, label, rec, imgNum) {
    ...
    const url = `/api/image/${(rec && rec._task_id) || currentTaskId}/${relPath(path)}`;
    return `
...
function buildCscanDetailHTML(rec) {
    const imgBase = `/api/image/${rec._task_id || currentTaskId}/`;
...
async function saveEditParams(e) {
    ...
    const newParams = {};
    ['材料尺寸', '缺陷中心X', '缺陷中心Y', '缺陷长度', '缺陷宽度', '缺陷深度'].forEach(k => {
        newParams[k] = form.elements[k].value.trim();
    });

    // 乐观更新 UI — 合并而非替换, 保留钢板号等非编辑字段
    const rec = recordsCache.find(r => r.row_index === currentEditRow);
    if (rec) {
        if (!rec['缺陷数据']) rec['缺陷数据'] = {};
        Object.assign(rec['缺陷数据'], newParams);
    }
...
async function refreshTaskList() {
    try {
        ...
        // 按厂分组
        const groups = {
            zhongban: { name: '中板厂 (模板一)', tasks: [], total: 0 },
            cscan: { name: '中厚板卷厂 (模板二)', tasks: [], total: 0 },
            kuanhouban: { name: '宽厚板厂 (模板三)', tasks: [], total: 0 },
        };
        for (const t of data.tasks) {
            const g = groups[t.task_type] || groups.zhongban;
            g.tasks.push(t);
            if (t.status === 'completed') g.total += (t.count || 0);
        }
        ...
        for (const [key, g] of Object.entries(groups)) {
            ...
            // 汇总选项
            const shortName = g.name.split(' (')[0];  // 中板厂, 中厚板卷厂, 宽厚板厂
            html += `<option value="summary:${key}">📊 ${shortName} 汇总 (${g.total} 条)</option>`;
            for (const t of g.tasks) {
                ...
                const statusLabel = t.status === 'completed' ? '✓'
                                  : t.status === 'failed' ? '✗'
                                  : t.status === 'processing' ? '⏳' + prog
                                  : '⏸';
                const sheetTag = t.sheet_name ? `[${t.sheet_name}]` : '';
                const file = (t.file || '').split('/').pop();
                html += `<option value="${t.task_id}">${escapeHtml(statusLabel + ' ' + sheetTag + count + ' ' + file)}</option>`;
            }
            html += '</optgroup>';
        }
...
function _setFactoryLabel(taskType, isSummary) {
    const labels = { zhongban: '中板厂', cscan: '中厚板卷厂', kuanhouban: '宽厚板厂' };
    const label = (labels[taskType] || taskType) + (isSummary ? ' 汇总 ' : ' ');
    const el = document.getElementById('result-factory-label');
    const el2 = document.getElementById('cscan-result-factory-label');
...
async function selectTask(taskId) {
    ...
    // 汇总选项
    if (taskId.startsWith('summary:')) {
        const taskType = taskId.split(':')[1];
        loadSummary(taskType);
        return;
    }
    ...
    _setFactoryLabel(taskType);

    if (taskType === 'cscan' || taskType === 'kuanhouban') {
        loadCscanRecords(taskId);
    } else {
        loadCscanRecords(null);
        loadRecords();
    }
}

async function loadSummary(taskType) {
    try {
        const res = await fetch(`/api/summary/${taskType}`);
        const data = await res.json();
        if (data.error) { alert(data.error); return; }

        _setFactoryLabel(taskType, true);

        if (taskType === 'zhongban') {
            ...
            renderRecords();
            document.getElementById('result-section').style.display = 'block';
            if (document.getElementById('cscan-section')) document.getElementById('cscan-section').style.display = 'none';
        } else {
            ...
            currentTaskType = taskType === 'kuanhouban' ? 'kuanhouban' : 'cscan';
            paginationState.page = 1;
            rebuildFilterDistinct();
            renderRecords();
            document.getElementById('result-section').style.display = taskType === 'zhongban' ? 'block' : 'none';
            const csec = document.getElementById('cscan-section');
            if (csec) csec.style.display = (taskType !== 'zhongban') ? 'block' : 'none';
        }
    } catch (e) {
...
function getDownloadBaseName() {
    const task = taskListCache.find(t => t.task_id === currentTaskId);
    if (task && task.file) {
        // task.file 是文件名 (如 "data.xlsx"), 去掉扩展名
        return task.file.replace(/\.[^.]+$/, '');
    }
    return currentTaskId; // 回退
}
...
document.getElementById('download-json').addEventListener('click', () => {
    ...
    downloadBlob(JSON.stringify(payload, null, 2), baseName + '_缺陷记录.json', 'application/json');
});

// zhongban Excel 导出 — fetch + blob, 支持自定义下载文件名
document.getElementById('download-zip')?.addEventListener('click', () => {
    if (!currentTaskId || currentTaskId.startsWith('summary:')) return;
...
document.getElementById('download-excel').addEventListener('click', async () => {
    ...
    const baseName = getDownloadBaseName();
    try {
        const resp = await fetch(`/api/image/${currentTaskId}/defect_records.xlsx`);
        if (!resp.ok) { alert('导出失败'); return; }
        const blob = await resp.blob();
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = baseName + '_缺陷记录.xlsx';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        setTimeout(() => URL.revokeObjectURL(url), 1000);
...
document.getElementById('cscan-download-zip')?.addEventListener('click', () => {
    if (!currentTaskId || currentTaskId.startsWith('summary:')) return;
    window.location.href = `/api/download/${currentTaskId}`;
});
...
document.getElementById('cscan-download-json')?.addEventListener('click', () => {
    ...
    const baseName = getDownloadBaseName();
    downloadBlob(JSON.stringify(payload, null, 2), baseName + '_CScan记录.json', 'application/json');
});
document.getElementById('cscan-download-excel')?.addEventListener('click', async () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = getDownloadBaseName() + '_CScan记录.xlsx';
...
// ===== 删除任务 =====
function registerDeleteButton(btnId) {
    document.getElementById(btnId)?.addEventListener('click', async () => {
        if (!currentTaskId) return;
        if (!confirm('确定要删除这个任务吗？此操作不可撤销，将删除所有相关数据和文件。')) return;
        try {
            const resp = await fetch(`/api/task/${currentTaskId}`, { method: 'DELETE' });
...
registerDeleteButton('delete-task');
registerDeleteButton('cscan-delete-task');
