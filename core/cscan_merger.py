...
from .cscan_ocr import find_cscan_sheets
...
def merge_cscan_records(
    ...
    """从 xlsx 的日期 sheet 提取基础字段, 与 image_map/OCR 结果合并.

    sheet_name: 日期格式 sheet 名 (如 "5.1"), 为空时自动选第一个.
    """
    ...
    if sheet_name and sheet_name in wb.sheetnames:
        ...
        # 自动选 sheet 优先级: Sheet2 > 日期格式 > 最后一个
    ...
    # 自动检测数据起始行: 找第一个 序号 是数字的行 (通常是 row 7, 但也可能是 5)
    data_start = 7
