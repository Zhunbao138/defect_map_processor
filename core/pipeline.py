...
class ProcessConfig:
    ...
    # 文档类型: "zhongban" (中板厂, 默认) / "cscan" (中厚板卷厂) / "kuanhouban" (宽厚板厂).
    # cscan/kuanhouban 走独立流水线 (切图 + JSON + 独立 SQLite 表)
...
class ProcessPipeline:
    ...
    def run(self, progress_callback=None) -> ProcessResult:
        ...
        """
        # cscan / kuanhouban 分支: 独立处理链 (切子图 + OCR表格)
        if self.config.task_type in ("cscan", "kuanhouban"):
    ...
    def _run_cscan(self, progress_callback=None) -> ProcessResult:
        ...
        try:
            # ========== 阶段 1: 提取 + 切图 ==========
            report("extract", 0.0, "打开文件，读取嵌入图片...")
            image_map = extract_cscan_from_xlsx(file_path, images_dir, sheet_name=self.config.sheet_name)
            total_rows = len(image_map)
            sub_count = sum(
                sum(1 for k in ("table","ascan","cscan") if paths.get(f"F_{k}") or paths.get(f"G_{k}"))
                for paths in image_map.values()
            )
            report("extract", 0.7, f"提取 {total_rows} 行，切 {sub_count} 张子图")

            # ========== 阶段 2-3: 识别 (缺陷表格 + 板信息) ==========
            rec_label_c = "大模型" if self.config.recognition == "llm" else "OCR"
            report("ocr", 0.0, f"{rec_label_c} 开始识别缺陷表格...")
            if self.config.recognition == "llm":
            ...
            for idx, (row_idx, paths) in enumerate(sorted(image_map.items())):
                ...
                report("ocr", 0.1 + 0.7 * (idx + 1) / total, f"{rec_label_c} 表格 {idx + 1}/{total} 行")

            # ========== 自动重试识别失败的表 ==========
            # 重置 LLM 连接，确保重试不受之前连接的残留状态影响
            try:
                ...
                pass
            retry_count = 0
            for row_idx, tables in ocr_table_map.items():
                for prefix in ("F", "G"):
                    ocr_result = tables.get(prefix)
                    if ocr_result and len(ocr_result) == 1 and isinstance(ocr_result[0], dict) and "warning" in ocr_result[0]:
                        fp = image_map.get(row_idx, {}).get(f"{prefix}_table", "")
                        if fp and os.path.exists(fp):
                            retry = ocr_defect_table(fp)
                            if retry and not (len(retry) == 1 and isinstance(retry[0], dict) and "warning" in retry[0]):
                                tables[prefix] = retry
                                retry_count += 1
            if retry_count:
                report("ocr", 0.95, f"自动重试 {retry_count} 处 → 修复 {retry_count}")
            else:
                report("ocr", 0.95, "全部识别成功，无需重试")

            # ========== 阶段 4: merge ==========
            report("merge", 0.0, f"合并 {len(ocr_table_map)} 行数据...")
            ...
            report("merge", 0.7, f"合并完成，共 {len(records)} 条记录")

            # ========== 阶段 5: 保存 ==========
            report("save", 0.0, "保存 JSON...")
            json_path = save_json(records, output_dir)
            result.json_path = str(json_path)
            report("save", 0.35, f"JSON 已保存 ({len(records)} 条)")

            report("save", 0.4, "生成 Excel...")
            ...
            report("save", 0.7, f"Excel 已生成")

            db_count = 0
            if self.config.save_to_db:
                report("save", 0.75, "写入数据库...")
                ...
                report("save", 0.95, f"数据库已更新 ({db_count} 行)")
