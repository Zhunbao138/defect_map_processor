...
```
settings.json（腰带） → AGENTS.md（大脑） → MEMORY.md（笔记本） → Hooks（肌肉） → Skills/Agents（手）
    强制                      建议                    学习                 执行              扩展
```

**关键原则**：
- **Tell in AGENTS.md, enforce in settings and hooks**
- AGENTS.md中的"NEVER do X"应该移到hooks
- AGENTS.md是建议，不是保证
...
**最重要的内容**：
1. **Common Pitfalls** - Codex常犯的错误（最高ROI）
...
```json
{
  "hooks": {
    "PreToolUse": [{
      "matcher": "Bash",
      "hooks": [{ "type": "command", "command": ".Codex/hooks/enforce-pnpm.sh" }]
    }]
  }
}
```
...
### 官方资源
- [Codex最佳实践](https://codewithclaude.net/advanced-topics/best-practices) - Anthropic工程师实战
- [AGENTS.md权威指南](https://potapov.dev/blog/Codex-md-guide) - 5层配置系统
- [E2E测试指南](https://shipyard.build/blog/e2e-testing-Codex/)

### 高质量Skills
1. **AI Workflow Skills（70+）** - [GitHub](https://github.com/ARazaAnjum/ClaudeCodeSkilledAgents)
   ```bash
   Codex plugin marketplace add ARazaAnjum/ClaudeCodeSkilledAgents
   Codex plugin install ai-workflow-skills@Codex-skilled-agents
   ```
...
3. **Codex Workflow Plugin** - Spec驱动开发
   ```bash
   Codex plugin marketplace add https://github.com/sighup/Codex-workflow.git
   ```
...
# Spec驱动开发
Codex plugin marketplace add https://github.com/sighup/Codex-workflow.git
Codex plugin install Codex-workflow@Codex-workflow --scope project
...
2. **简洁优先**：每次变更最小化影响
3. **测试优先**：TDD是默认方式
4. **配置分层**：Tell in AGENTS.md, enforce in hooks
5. **持续改进**：错误 → 记录 → 规则 → 验证
6. **验证完成**：未验证不标记完成

---

*每行都要有价值。如果删除某行不会让Codex的输出变差，就删掉它。*
